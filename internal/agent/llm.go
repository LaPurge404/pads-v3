...
func (c *ClaudeClient) doGenerate(ctx context.Context, prompt CodePrompt) (*CodeResponse, error) {
	...
	body, statusCode, err := c.doRequest(ctx, "https://api.anthropic.com/v1/messages", payload)
	if err != nil {
		return nil, err
	}
	...
	if len(claudeResp.Content) == 0 || claudeResp.Content[0].Type != "text" {
		return nil, fmt.Errorf("unexpected Claude response structure")
	}

	return &CodeResponse{
		Patch:       claudeResp.Content[0].Text,
		Explanation: fmt.Sprintf("Generated by Claude (%s) for task: %s", c.Model, prompt.Task),
		Confidence:  0.7,
	}, nil
}

// doRequest sends an authenticated POST request to the Claude API.
func (c *ClaudeClient) doRequest(ctx context.Context, url string, payload []byte) (body []byte, status int, err error) {
...
// mockResponse returns a mock response for development without API keys.
func (c *ClaudeClient) mockResponse(prompt CodePrompt) *CodeResponse {
	return &CodeResponse{
