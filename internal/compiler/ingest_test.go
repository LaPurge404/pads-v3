...
`
    ...
    if len(nodes1) != len(nodes2) { t.Errorf("nodes count mismatch") }
    if len(edges1) != len(edges2) { t.Errorf("edges count mismatch") }
    for i := range nodes1 { if nodes1[i] != nodes2[i] { t.Errorf("node mismatch") } }
    for i := range edges1 { if edges1[i] != edges2[i] { t.Errorf("edge mismatch") } }
...
`), 0644)
    ...
    if h1 == h2 { t.Errorf("hash should differ for different signatures") }
...
`
    ...
    if countNodes(t, db, testFile) != 0 { t.Errorf("nodes not cleared") }
    res2, _ := IngestFile(db, testFile)
    if res1.NodesAdded != res2.NodesAdded || res1.EdgesAdded != res2.EdgesAdded {
...
`
    ...
    foundExternal := false
    for _, c := range calls {
        if c == "main.B" { foundB = true }
        if c == "main.Println" { foundExternal = true }
    }
    if !foundB { t.Error("missing CALLS to local function B") }
    if foundExternal { t.Error("external call to Println should be ignored") }
...
`
    ...
    found := false
    for _, n := range nodes {
        if n == "main.*Server.Start" { found = true }
    }
    if !found { t.Error("method not extracted") }
